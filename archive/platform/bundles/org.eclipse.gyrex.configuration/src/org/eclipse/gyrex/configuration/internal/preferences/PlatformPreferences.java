	...
	protected PlatformPreferences(final IEclipsePreferences parent, final String name, final boolean removed) {
		...
		if ((null != parent) && (null != name)) {
			if (parent instanceof PlatformPreferences) {
				...
				instanceNode = (IEclipsePreferences) ConfigurationActivator.getInstance().getPreferencesService().getRootNode().node(InstanceScope.SCOPE).node(ConfigurationActivator.PLUGIN_ID).node(name);
			}
		} else {
			instanceNode = null;
		}
