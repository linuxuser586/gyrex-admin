...
import org.eclipse.gyrex.admin.ui.internal.helper.StringMatcher;
...
public class FilteredList extends Composite {
	...
	private static class Label {
		 ...
		 * @param image
		 */
		public Label(final String newString, final Image image) {
			if (newString == null) {
	...
	private final class LabelComparator implements Comparator<Object> {
		private final boolean labelIgnoreCase;

		LabelComparator(final boolean ignoreCase) {
			labelIgnoreCase = ignoreCase;
		}

		@Override
		public int compare(final Object left, final Object right) {
			final Label leftLabel = (Label) left;
			final Label rightLabel = (Label) right;
			int value;
			if (fComparator == null) {
	...
	private class TableUpdateJob extends UiJob {
		final Table fTable;
		 ...
		 */
		public TableUpdateJob(final Table table, final int count) {
			super(table.getDisplay(), WidgetMessages.get().FilteredList_UpdateJobName);
			setSystem(true);
			fTable = table;
			fCount = count;
		}
		...
		public IStatus runInUIThread(final IProgressMonitor monitor) {
			...
			final int iterations = Math.min(10, fCount - currentIndex);
	 ...
	 */
	public FilteredList(final Composite parent, final int style, final ILabelProvider labelProvider, final boolean ignoreCase, final boolean allowDuplicates, final boolean matchEmptyString) {
	...
	private int fold() {
		...
		Label last = null;
		for (int i = 0; i != fFilteredCount; i++) {
			final int j = fFilteredIndices[i];
			final Label current = fLabels[j];
	...
	public Object[] getFoldedElements(final int index) {
		if ((index < 0) || (index >= fFoldedCount))
			return null;
		final int start = fFoldedIndices[index];
		final int count = (index == (fFoldedCount - 1)) ? fFilteredCount - start : fFoldedIndices[index + 1] - start;
	...
	public Object[] getSelection() {
		if (fList.isDisposed() || (fList.getSelectionCount() == 0))
			return new Object[0];
		final int[] indices = fList.getSelectionIndices();
		final Object[] elements = new Object[indices.length];
	...
	public void setComparator(final Comparator<?> comparator) {
		...
		fComparator = (Comparator<Object>) comparator;
	}
	...
	public void setElements(final Object[] elements) {
		...
		final int length = fElements.length;
		// fill labels
		fLabels = new Label[length];
		final Set<Image> imageSet = new HashSet<Image>();
		for (int i = 0; i != length; i++) {
			final String text = fLabelProvider.getText(fElements[i]);
			final Image image = fLabelProvider.getImage(fElements[i]);
	 ...
	 * @param filterMatcher
	 */
	public void setFilterMatcher(final FilterMatcher filterMatcher) {
	 ...
	 */
	public void setSelection(final Object[] elements) {
		...
		for (int i = 0; i != elements.length; i++) {
			...
			for (j = 0; j != fFoldedCount; j++) {
				final int max = (j == (fFoldedCount - 1)) ? fFilteredCount : fFoldedIndices[j + 1];
				int l;
