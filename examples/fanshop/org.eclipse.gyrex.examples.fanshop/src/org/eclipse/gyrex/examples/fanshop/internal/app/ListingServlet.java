...
public class ListingServlet extends HttpServlet {
	...
	protected void doGet(final HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
		...
		if (results.size() < results.getNumFound()) {
			...
			if (results.getStart() == 0) {
				writer.println("Only products the first " + results.size() + " products are shown.");
			} else {
	...
	private void writeListing(final SolrDocument listing, final PrintWriter writer, final HttpServletRequest req) {
		...
		writer.print(MeasureFormat.getCurrencyFormat(ULocale.US).format(new CurrencyAmount((Double) listing.getFirstValue("price"), com.ibm.icu.util.Currency.getInstance("USD"))));
